        ...
        { 
            ...
            case STATE5:
                if(((parcels[1])^(parcels[2]))==(parcels[3])){
                    printf("\nSET message received without errors\n\n");
                    STOP = TRUE;
                }
                else {
                    st = STATE0;
...
int llwrite(const unsigned char *buf, int bufSize)
{
    ...
    //3º criar a nova I com o dataPacket (ja stuffed) la dentro
    //4º enviar a I e contar o alarme
    ...
    unsigned char BCC = 0x00, I[600] = {0}, parcels[5] = {0};
...
{   
    ...
    unsigned char I[600]={0}, supFrame[5]={0}, BCC2=0x00, aux[400] = {0}, flagCount = 0, STOP = FALSE; 
    ...
    if((I[1]^I[2]) != I[3] || I[2] != control){
        printf("\nI not received correctly. Protocol error. Sending REJ.\n");
    ...
    for(int i=0; i<sizeInfo; i++){
        if(I[i] == 0x7D && I[i+1]==0x5e){
            packet[index++] = 0x7E;
            i++;
        }

        else if(I[i] == 0x7D && I[i+1]==0x5d){
            packet[index++] = 0x7D;
            i++;
        }

        else {packet[index++] = I[i];}
    }
    ...
    if(packet[size-2] == BCC2){
        ...
        if(packet[4]==0x01){
            if(I[5] == lastFrameNumber){
                printf("\nI received correctly. Repeated Frame. Sending RR.\n");
        ...
        printf("\nI received correctly. Sending RR.\n");
        supFrame[2] = (receiverNumber << 7) | 0x05;
        supFrame[3] = supFrame[1] ^ supFrame[2];
        write(fd, supFrame, 5);
    }
        ...
        printf("\nI not received correctly. Error in data packet. Sending REJ.\n");
        supFrame[2] = (receiverNumber << 7) | 0x01;
        supFrame[3] = supFrame[1] ^ supFrame[2];
        write(fd, supFrame, 5);
